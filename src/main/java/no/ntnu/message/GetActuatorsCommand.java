...
public record GetActuatorsCommand(int nodeId) implements ListCommand {
   ...
   */
  @Override
  public List<ActuatorStateMessage> execute(GreenhouseSimulator logic) {
    return logic.getNode(nodeId).getActuators().stream()
        .map(actuator -> new ActuatorStateMessage(nodeId, actuator.getId(), actuator.isOn(), actuator.getStrength(), actuator.getMinStrength(), actuator.getMaxStrength(), actuator.getUnit(), actuator.getType()))
        .toList();
  }
}
